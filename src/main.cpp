...
#if defined(NDEBUG)
# error "wrapper cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa4d6fc138e2ab2fa649e97472c481009f6761548cbf7d86a14ca0ddc1e74203c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 8); // wrapper: starting difficulty is 1 / 2^12
...
int64 CTransaction::nMinTxFee = 1000000; // 0.01 wrapper
/** Fees smaller than this (in ztoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 1000000; // 0.01 wrapper
...
const string strMessageMagic = "Wrapper Signed Message:\n";
...
{
    // Wrapper: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nTime)
{
    ...
    {
        // First 250 blocks (excluding the ICO one) are low reward
        if (nHeight < 250)
            nSubsidy = 1 * COIN;
        else if(nHeight >= 250 && nHeight < 2000000)
           nSubsidy = (20 - 0.000005*nHeight) * COIN;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    static const uint32_t        BlocksTargetSpacing                        = 12 * 60; // 10 minutes
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot, bool isVerifyDB) const
{
    ...
    // Wrapper: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Wrapper: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Fidel Castro, Cuba's leader of revolution, dies at 90";
        CTransaction txNew;
        ...
        block.nVersion = 2;
        block.nTime    = 1480155869;
        block.nBits    = startBits;
        block.nNonce   = 3450412;

        if (fTestNet)
        {
            block.nTime    = 1480153823;
        ...
        if(fTestNet)
            ...
            genMerkleRoot.SetHex("0x67e92381aa214f977a6379a1901809b030670792d3373a427f6a171617bc5659");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WrapperMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WrapperMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("WrapperMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Wrapper-Miner");
